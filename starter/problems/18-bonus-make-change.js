...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  function makeChange(target, coins) {
    ...
    for (let i = 0; i < coins.length; i++) {
      let num = coins[i];
      let result = makeChange(target - num, coins)
  ...
  let result = makeChange(target, coins);

  return (result > 0) ? maxCoins : null;
}
...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  ...
  for (let i = 0; i < coins.length; i++) {
      ...
      if (currentCoin <= target) {
          const changeWithoutCurrentCoin = makeBetterChange(target - currentCoin, coins.slice(i));

          if (changeWithoutCurrentCoin !== null) {
              const change = [...changeWithoutCurrentCoin, currentCoin];
